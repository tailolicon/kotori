...
abstract class YoutubeChannelSource(
    ...
    override suspend fun resolveVideos(episode: SEpisode): List<Video> = withContext(Dispatchers.IO) {
        ...
        // Auto-generated (ASR) captions are unreliable to fetch (often time out or 404 on mpv's
        // sub-add, delaying/blocking playback start) and are rarely the right language anyway —
        // Muse/Ani-One episodes are already hard-dubbed in Vietnamese. Only keep real, authored
        // tracks.
        val subtitles = info.subtitles.mapNotNull { sub ->
            if (sub.isAutoGenerated) return@mapNotNull null
