...
class AnimeUpdatesScreenModel(
    ...
    init {
        screenModelScope.launchIO {
            ...
            combine(
                ...
                filterPreferenceFlow().distinctUntilChanged(),
            ) { updates, _, _, prefs -> updates to prefs }
                .catch {
                    logcat(LogPriority.ERROR, it)
                    _events.send(Event.InternalError)
                }
                .collectLatest { (updates, prefs) ->
                    mutableState.update {
                        it.copy(
                            isLoading = false,
                            items = updates.toUpdateItems().applyFilters(prefs),
                        )
                    }
                }
        }

        filterPreferenceFlow()
            .map { prefs ->
                listOf(
                    prefs.filterUnread,
            ...
            .onEach { active -> mutableState.update { it.copy(hasActiveFilters = active) } }
    ...
    private fun filterPreferenceFlow(): kotlinx.coroutines.flow.Flow<FilterPrefs> = combine(
        updatesPreferences.filterDownloaded.changes(),
        updatesPreferences.filterUnread.changes(),
        updatesPreferences.filterStarted.changes(),
        updatesPreferences.filterBookmarked.changes(),
    ) { downloaded, unread, started, bookmarked ->
        FilterPrefs(downloaded, unread, started, bookmarked)
    }

    private fun PersistentList<AnimeUpdatesItem>.applyFilters(prefs: FilterPrefs): PersistentList<AnimeUpdatesItem> {
        return this
            .filterIfActive(prefs.filterDownloaded) {
                it.downloadStateProvider() == AnimeDownload.State.DOWNLOADED
            }
            .filterIfActive(prefs.filterUnread) { !it.update.seen }
            .filterIfActive(prefs.filterStarted) { it.update.seen || it.update.lastSecondSeen > 0L }
            .filterIfActive(prefs.filterBookmarked) { it.update.bookmark }
            .toPersistentList()
    }

    private inline fun List<AnimeUpdatesItem>.filterIfActive(
        state: TriState,
        predicate: (AnimeUpdatesItem) -> Boolean,
    ): List<AnimeUpdatesItem> = when (state) {
        TriState.DISABLED -> this
        TriState.ENABLED_IS -> filter(predicate)
        TriState.ENABLED_NOT -> filterNot(predicate)
    }
