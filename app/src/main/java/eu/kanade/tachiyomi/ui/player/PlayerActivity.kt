...
class PlayerActivity : BaseActivity() {
    ...
    fun setHttpOptions(video: Video) {
        ...
        // Forward session cookies for the stream host so CDNs that gate streams behind a
        // cookie don't return an HTML error page (mpv: "Failed to recognize file format").
        video.videoUrl.toHttpUrlOrNull()?.let { httpUrl ->
            val cookies = Injekt.get<NetworkHelper>().cookieJar.loadForRequest(httpUrl)
            if (cookies.isNotEmpty()) {
                val cookieHeader = cookies.joinToString("; ") { "${it.name}=${it.value}" }
                val existing = headers["Cookie"]
                headers["Cookie"] = if (existing.isNullOrBlank()) {
                    cookieHeader
                } else {
