...
class AniyomiMPVView(context: Context, attributes: AttributeSet) : BaseMPVView(context, attributes) {
    ...
    override fun initOptions(vo: String) {
        ...
        // Demuxer cache: 64 MiB was too small — the in-memory cache filled after ~10 min of a
        // stream and playback stalled (every episode cut off around 9-10 min). Use a larger
        // in-memory read-ahead window and spill the cache to disk so full episodes stream through.
        ...
        val mpvCacheDir = File(context.cacheDir, "mpv-cache").apply { mkdirs() }
        MPVLib.setOptionString("cache", "yes")
        MPVLib.setOptionString("cache-on-disk", "yes")
        MPVLib.setOptionString("cache-dir", mpvCacheDir.path)
        //
