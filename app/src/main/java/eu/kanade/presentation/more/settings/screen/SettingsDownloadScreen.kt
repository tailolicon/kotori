...
object SettingsDownloadScreen : SearchableSettings {
    ...
    override fun getPreferences(): List<Preference> {
        ...
        val shared = mutableListOf<Preference>(
        ...
        if (!isAnime) {
            shared += Preference.PreferenceItem.SwitchPreference(
        ...
        shared += Preference.PreferenceItem.SliderPreference(
        ...
        if (isAnime) {
            shared += getDeleteEpisodesGroup(downloadPreferences, allCategories)
            shared += getAutoDownloadEpisodesGroup(downloadPreferences, allCategories)
            shared += getDownloadAheadEpisodesGroup(downloadPreferences)
        } else {
            shared += getDeleteChaptersGroup(downloadPreferences, allCategories)
            shared += getAutoDownloadGroup(downloadPreferences, allCategories)
            shared += getDownloadAheadGroup(downloadPreferences)
        }
        return shared
    }
    ...
    @Composable
    private fun getAutoDownloadEpisodesGroup(
        downloadPreferences: DownloadPreferences,
        allCategories: List<Category>,
    ): Preference.PreferenceGroup {
        val downloadNewEpisodesPref = downloadPreferences.downloadNewEpisodes()
        val downloadNewUnseenOnlyPref = downloadPreferences.downloadNewUnseenEpisodesOnly()
        val includedPref = downloadPreferences.downloadNewEpisodeCategories()
        val excludedPref = downloadPreferences.downloadNewEpisodeCategoriesExclude()

        val downloadNewEpisodes by downloadNewEpisodesPref.collectAsState()
        val included by includedPref.collectAsState()
        val excluded by excludedPref.collectAsState()
        var showDialog by rememberSaveable { mutableStateOf(false) }
        if (showDialog) {
            TriStateListDialog(
                title = stringResource(MR.strings.categories),
                ...
                onDismissRequest = { showDialog = false },
                onValueChanged = { newIncluded, newExcluded ->
                    includedPref.set(newIncluded.fastMap { it.id.toString() }.toSet())
                    excludedPref.set(newExcluded.fastMap { it.id.toString() }.toSet())
                    showDialog = false
                },
            )
        }
        ...
        return Preference.PreferenceGroup(
            ...
            preferenceItems = listOf(
                ...
                Preference.PreferenceItem.SwitchPreference(
                    preference = downloadNewUnseenOnlyPref,
    ...
    @Composable
    private fun getDownloadAheadEpisodesGroup(
        downloadPreferences: DownloadPreferences,
    ): Preference.PreferenceGroup {
        return Preference.PreferenceGroup(
            title = stringResource(MR.strings.download_ahead),
