...
/**
 * An [AnimeHttpSource] that resolves an episode straight to its playable video(s).
 *
 * A subclass of [AnimeHttpSource] must implement the hoster hooks, which is what makes the
 * player's `EpisodeLoader.checkHasHosters` route it through the hoster API rather than the legacy
 * `getVideoList(episode)` path. Sources that have nothing to choose between — one embed, one
 * stream — implement [resolveVideos] instead and this wraps the result in a single hoster.
 */
abstract class HosterAnimeSource : AnimeHttpSource() {
