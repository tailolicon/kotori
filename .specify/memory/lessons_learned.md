...
- **Enforced Solution:** Merge chapter/page and episode/time positions with the maximum local/remote value, enqueue immediately from reader/player persistence call sites, replace superseded full-snapshot work instead of appending a backlog, upload via temporary PUT plus atomic MOVE, and validate/fall back to dated snapshots before restore; broad database observers also see restore writes and create sync feedback loops.
